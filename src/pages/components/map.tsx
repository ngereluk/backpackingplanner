...
const KananaskisMap = (props: innerMapProps) => {
  const [staticRouteData, setStaticRouteData] = useState([]);
  ...
  async function getStaticRouteData() {
    const data = await fetch("/api/getStaticRouteDataForMap", {
      method: "GET",
      headers: {
        "Content-type": "application/json",
      },
  ...
  useEffect(() => {
    (async () => {
      const staticRouteData = await getStaticRouteData();
      if (staticRouteData != undefined) {
        setStaticRouteData(staticRouteData);
      }
    })();
  }, []);
